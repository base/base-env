...
function EnvPath(name, env, app) {
  ...
  this.define('fn', {
    get: function() {
      ...
      if (utils.isObject(fn)) {
        if (!fn.isBase) {
          throw new Error('expected a function or instance to be exported from: ' + this.path);
        }
        // since `fn` is an instance, update `env.instance` to be `fn`
        this.app = this.instance = fn;
        fn = function() {};
        this._fn = fn;
      }
      this.setup(fn, this.app);
      return fn;
    }
  });
  ...
  utils.alias(this, app);
  utils.namespace(this, app);
}

/**
 * Run `env.fn.setup` function with `runnerContext`
 */

EnvPath.prototype.setup = function(fn, app) {
  if (fn && typeof fn.setup === 'function') {
    var ctx = app.base.get('cache.runnerContext');
    if (utils.isObject(ctx) && ctx.base) {
      fn.setup(app, ctx.base, ctx);
    }
  }
};
...
EnvPath.prototype.inspect = function() {
  return '<Env "' + this.namespace + '" [' + utils.pathName(this) + ']>';
};
